...
void WorldMap::Initializing(const char** array)
{
...
void WorldMap::set_RandomSprites(const char*** Sprite_Set, Sprite_RandSet& sprite_obj)
{
    for(int posIndex_X = 0; posIndex_X < ((map_height - 2) / 7); ++posIndex_X)
        for(int posIndex_Y = 0; posIndex_Y < ((map_width - 3) / 7); ++posIndex_Y)
        {
            // rand call
...
void WorldMap::Render(Map_Objects** Map_Objects_Pointer, Shader& ourShader, unsigned int VAO, unsigned int texture1)
{
